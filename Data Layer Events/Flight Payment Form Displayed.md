...
digitalEventData.push({
  "event": "Flight Payment Form Displayed",
    ...
    "westJetEvents": {
        "cciaBannerImpression": <cciaBannerImpression>,
        ...
        "minCreditBannerImpression": <minCreditBannerImpression>,
        "wsdTbBannerImpression": <wsdTbBannerImpression>,
        "wsdTbCcBannerImpression": <wsdTbCcBannerImpression>
    }
});
```
...
|westJetEvents.cciaBannerImpression|boolean|Used to indicate when the CCIA banner is displayed on the Payment page.|TRUE, FALSE|||||||
...
|westJetEvents.minCreditBannerImpression|boolean|Used to indicate when the “Min Credit Card”variation of the WSD + TB + CC banner contextual message appears on payment FOP.NOTE: This event must fire in conjunction with event522 ONLY if the“Min Credit Card”banner is displayed.|TRUE, FALSE|||||||
|westJetEvents.wsdTbBannerImpression|boolean|Used to indicate when the WSD + TB banner contextual message appears on the Payment page.|TRUE,FALSE|||||||
|westJetEvents.wsdTbCcBannerImpression|boolean|Used to indicate when the WSD + TB+ CC banner contextual message is clicked on the Payment page.|TRUE, FALSE|||||||
